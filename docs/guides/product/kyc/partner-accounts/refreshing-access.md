...
Source: https://docs.wise.com/guides/product/kyc/partner-accounts/refreshing-access
...
Ownership of the business profile was transferred to another Wise account.
The user enables enhanced security on their Wise account.
Wise revoked a token due to a suspected security breach of the token or your client secret.
...
Changing Business Profile ownership

In the event your organization transfers ownership of the business profile from one Wise user account to another, you will need to go through the oAuth linking procedure with the new Wise user account. A change in profile ownership like this will invalidate existing access and refresh tokens.

The new user account will need to visit the link below, log in, and reauthorize accesss to the account.

https://wise.com/oauth/authorize/?client_id={client_id}&redirect_uri={your_redirect_url}

After reauthorization, Wise will return a new authorization code as a parameter in the redirect url. Exchange that authorization code for a pair of oAuth tokens (access & refresh token). These tokens can be used by the new user account to transact as the business profile.
...
In this case please utilise the linking to an existing Wise account flow.